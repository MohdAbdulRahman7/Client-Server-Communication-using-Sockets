...
* Main Server Connection - Port 6999
*
...
// Close the pipe and get the exit status of the command
    ...
    // Check the exit status to determine if any files were found
    if (WIFEXITED(status)) {
        int exit_status = WEXITSTATUS(status);
        if (exit_status == 0) {
            // Files were found, send response
            sprintf(response, "Archive created: temp.tar.gz\n");
        } else {
            // No files found, send response
            sprintf(response, "No files found.\n");
        }
    } else {
        // pclose() did not return a normal exit status
        sprintf(response, "Error executing command.\n");
    }
...
/**********/
// Open the file_list.txt for reading
    file_list = fopen("file_list.txt", "r");
    if (file_list == NULL) {
        perror("fopen");
        exit(EXIT_FAILURE);
    }
    ...
    while ((ch = fgetc(file_list)) != EOF) {
        if (!isspace(ch)) { // Checking for non-whitespace characters
            is_empty = 0;
            break;
        }
    }
    ...
    // If file is empty, send "No files found" response
    if (is_empty) {
        sprintf(response, "No files found.\n");
...
int main(int argc, char *argv[]) {
  int sockfd, newsockfd, portno = SERVER_PORT;    // socket fds -  individual client connections
  ...
  while (1) {
    ...
    // Main Server
    if ((conn_id >= 1 && conn_id <= 3) ||
        ((conn_id > 9) && ((conn_id - 10) % 3 == 0))) {
      pid = fork();
      if (pid < 0)
      ...
      if (pid == 0) {
        ...
        close(newsockfd);
        conn_id++; // Increment the connection count
      }
    }

    // Mirror1
    else if ((conn_id >= 4 && conn_id <= 6) ||
             ((conn_id > 9) && ((conn_id - 10) % 3 == 1))) {
      pid = fork();
      if (pid < 0)
        caught_error("ERROR: Failed while forking");
      if (pid == 0) {
        close(sockfd);
        printf("Handling connection %d\n", conn_id);
        redirect_to_mirror(newsockfd, MIRROR1_PORT);
        // close(newsockfd);
        exit(EXIT_SUCCESS);
      } else {
        close(newsockfd);
    ...
    // Mirror2
    else if ((conn_id >= 7 && conn_id <= 9) ||
             ((conn_id > 9) && ((conn_id - 10) % 3 == 2))) {
      pid = fork();
      if (pid < 0)
        caught_error("ERROR: Failed while forking");
      if (pid == 0) {
        close(sockfd);
        printf("Handling connection %d\n", conn_id);
        redirect_to_mirror(newsockfd, MIRROR2_PORT);
        // close(newsockfd);
        exit(EXIT_SUCCESS);
      } else {
        close(newsockfd);
